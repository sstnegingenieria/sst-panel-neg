...
import CatalogoPanel from '../../components/sigp/catalogo/CatalogoPanel'
...
export default function CotizacionesSigp() {
  ...
  // el filtro de pendientes de diligenciar (aterriza en la pestaña Cotizaciones).
  // Reorg de navegación: el catálogo NEG vive como pestaña (?tab=catalogo) —
  // el estado va en la URL para que el redirect de /sigp/catalogo y los
  // enlaces guardados funcionen.
  const [searchParams, setSearchParams] = useSearchParams()
  const tabActiva: 'cotizaciones' | 'catalogo' =
    searchParams.get('tab') === 'catalogo' ? 'catalogo' : 'cotizaciones'
  const cambiarTab = (tab: 'cotizaciones' | 'catalogo') => {
    const next = new URLSearchParams(searchParams)
    if (tab === 'catalogo') next.set('tab', 'catalogo')
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {tabActiva === 'catalogo'
              ? 'Catálogo NEG · ítems propios incorporados desde el cotizador, mantenimiento trazado en el historial'
              : 'Ofertas al cliente · versiones y aprobación'}
          </p>
        </div>
        {tabActiva === 'cotizaciones' && puedeGestionar && (
      ...
      {/* Segmented Cotizaciones · Catálogo NEG (reorg de navegación — el
          catálogo dejó de ser página propia; permisos DENTRO del panel) */}
      <div className="inline-flex rounded-lg border border-gray-200 bg-gray-50 p-0.5 text-sm font-medium">
        <button
          onClick={() => cambiarTab('cotizaciones')}
          className={`px-4 py-1.5 rounded-md transition ${
            tabActiva === 'cotizaciones' ? 'bg-white text-brand-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
          Cotizaciones
        </button>
        <button
          ...
          className={`px-4 py-1.5 rounded-md transition ${
            tabActiva === 'catalogo' ? 'bg-white text-brand-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
          📚 Catálogo NEG
        </button>
      ...
      {tabActiva === 'catalogo' && <CatalogoPanel />}
