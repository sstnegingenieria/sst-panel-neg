...
CF con prefijo PEA y reglas FS/ST desplegadas aditivas antes del merge).
**DECISIÓN DE NEGOCIO CERRADA** (Giovanny, 18-ago, PR #97): `operacion_comercial`
NO ve actividades ni propuestas — Karen está a cargo únicamente de licitaciones;
la operación in-house de Claro no es su frente. Helper propio del módulo
`gestionaActividades()` en las DOS colecciones (`puedeGestionarProyectos()`
intacto). Queda abierto solo: la ratificación de Ingrid del CM-FT-CT-19
numerado PEA- (frente Trinorma) y el estreno. Bloque aparte del PR C
