...
- **1.4-B ⏳ Cotizaciones (complementos)** — siguiente sub-iteración: constructor de ítems APU (análisis de precios unitarios), generación del PDF de la cotización (pdf-lib, plantilla CM-FT-CT-19, hash SHA-256) y refinamientos que Giovanny difirió de la validación de 1.4-A.
- **1.5 ⏸ Refinamientos comerciales (pendiente — NO iniciada)** — registrada al cierre de F1.4-B para que no se pierda. Cuatro puntos validados por Giovanny (11-jul-2026):
  1. **Evaluador de expresiones matemáticas en campos numéricos** (cantidades, rendimientos, factores): aceptar `20.23*5`, `1/54`, `(15+3)*2` → evaluar al confirmar y guardar el resultado. **NO usar `eval`**; usar **mathjs** acotado a `+ − * /` y paréntesis (⚠ mathjs NO está instalado hoy — dependencia pre-aprobada para F1.5).
  2. **Agrupador por ACTIVIDADES como entidad propia** — ⚠ **CAMBIO DE MODELO, requiere sesión de diseño**: actividades con nombre libre editable, un ítem puede repetirse en varias actividades, y una actividad contiene ítems de capítulos distintos. Lo implementado en 1.4B.d (`agrupador` en la versión) es solo un reetiquetado del campo `capitulo` del ítem — insuficiente para este alcance. **Es el punto de fondo de F1.5.**
  3. **Numeración de versiones de cara al cliente**: V1 invisible (la primera emisión sale sin versión visible); desde la primera modificación se muestra V2, V3… Solo presentación (UI + PDF); el `version: 1` interno se mantiene.
  4. **Máximo 2 decimales en toda la UI** (cantidades, valores, porcentajes, rendimientos, totales); la precisión completa se conserva internamente (snapshots y cálculos no cambian).
...
*Última actualización de este archivo: 11 de julio de 2026, tras el cierre de F1.4-A (Cotizaciones — núcleo). Versión: 1.4.0.*
