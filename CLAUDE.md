...
*Última actualización de este archivo: 20 de julio de 2026 — **F2 COMPLETO Y LISTO PARA GO-LIVE** (F2.1: proyecto de la aprobación al handoff a facturación · F2.2: entrada de preventivos IHS con matriz de precios y cliente fijo; todo tras `sigp_f2_enabled` default false). F1 sigue EN PRODUCCIÓN con el piloto comercial activo. **Go-live pendiente**: parámetro Remote Config `sigp_f2_enabled` + deploy de reglas Firestore (`proyectos` + segregación gerencia) y Storage (`proyectos/**`). Pendientes posteriores: F2.3, módulo Administrativa (factura/pagos/saldo con gate), ajustes del piloto, aviso de "versión nueva del panel", campo `celular`, H-008. UX: LPU relocalizado al detalle de Cliente (20-jul). Versión: 2.2.1.*
