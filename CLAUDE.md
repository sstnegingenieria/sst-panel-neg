...
*Última actualización de este archivo: 20 de julio de 2026 — **F2.1 COMPLETA** (a: ficha de Proyecto · b: asignación + permisos · c: preliquidación con margen APU y segregación · d: ejecución → entrega → soporte → handoff + evaluación del contratista; todo tras `sigp_f2_enabled` default false). F1 sigue EN PRODUCCIÓN con el piloto comercial activo. Siguiente hito: **go-live de F2.1** (parámetro Remote Config + deploy de reglas Firestore `proyectos`+segregación y Storage `proyectos/**`). Pendientes: F2.2/F2.3, módulo Administrativa (factura/pagos/saldo con gate), ajustes del piloto, aviso de "versión nueva del panel", campo `celular`, H-008. Versión: 2.1.3.*
