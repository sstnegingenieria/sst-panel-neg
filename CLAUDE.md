...
*Última actualización de este archivo: 19 de julio de 2026 — **F2.1.a, b y c HECHAS** (ficha de Proyecto, asignación con evidencia de habilitación + permisos, y preliquidación con margen tipo APU + segregación de funciones + PDF del contratista; todo tras `sigp_f2_enabled` default false). F1 sigue EN PRODUCCIÓN con el piloto comercial activo. Pendientes: F2.1.d (ejecución); al go-live de F2.1: parámetro Remote Config + deploy de reglas Firestore (`proyectos` + segregación gerencia) y Storage (`proyectos/**`); ajustes del piloto, aviso de "versión nueva del panel" tras deploys, campo `celular` en perfiles, H-008. Versión: 2.1.2.*
