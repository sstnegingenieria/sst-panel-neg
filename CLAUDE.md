...
3. **Deploy de `storage.rules`**: bloque `proyectos/**` auth-only (adjuntos de permisos de ingreso; nuevo en 2.1.b).
4. Functions: **nada** — el prefijo `PRY` ya está en la `generarConsecutivo` desplegada desde F0.
...
*Última actualización de este archivo: 18 de julio de 2026 — **F2.1.a y F2.1.b HECHAS** (ficha de Proyecto + asignación de contratista con evidencia de habilitación + permisos de ingreso, todo tras `sigp_f2_enabled` default false). F1 sigue EN PRODUCCIÓN con el piloto comercial activo. Pendientes: F2.1.c/d; al go-live de F2.1: parámetro Remote Config + deploy de reglas Firestore (`proyectos`) y Storage (`proyectos/**`); ajustes del piloto, aviso de "versión nueva del panel" tras deploys, campo `celular` en perfiles, H-008. Versión: 2.1.1.*
